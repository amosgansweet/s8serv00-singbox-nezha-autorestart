...
# 默认恢复命令
default_restore_command = " (ps aux | grep -v grep | grep amossb > /dev/null ) &&  (ps aux | grep -v grep | grep amoscf > /dev/null ) && (ps aux | grep -v grep | grep nezha-agent > /dev/null) || $HOME/sb/rt.sh >/dev/null 2>&1 &"
